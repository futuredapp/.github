...
@test "parses version tag with build number 1.2.3-42" {
  VERSION_TAG="1.2.3-42" run bash "$SCRIPT"
  [ "$status" -eq 0 ]
  grep -q "^version_number=1.2.3$" "$GITHUB_OUTPUT"
  grep -q "^build_number=42$" "$GITHUB_OUTPUT"
}
...
@test "parses large version numbers 10.20.30-999" {
  ...
  grep -q "^build_number=999$" "$GITHUB_OUTPUT"
}

@test "parses build number zero 1.0.0-0" {
  VERSION_TAG="1.0.0-0" run bash "$SCRIPT"
  [ "$status" -eq 0 ]
  grep -q "^version_number=1.0.0$" "$GITHUB_OUTPUT"
  grep -q "^build_number=0$" "$GITHUB_OUTPUT"
}

@test "rejects semver pre-release tag 1.0.0-beta.1" {
